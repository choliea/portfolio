    ...
    return {
      ...
      portfolio1: {
        ...
        description: `
          ...
          그중에서도 다른 일반 상품과 달리 같은 모델이더라도 제품의 사양이 다양하다는 점,<br>
          데이터를 다루는데 조금 더 많은 고민이 필요하다고 생각되는 전자제품을 선택하게 되었습니다. <br>
          저는 Product 파트를 맡았고, 제품 검색 기능, 최근 본 상품, 관심상품 서비스를 구현하였습니다. </p>
          
          <p>모델의 사양을 옵션 테이블로 분리하였고<br>
          모델 정보와 옵션 정보를 포함하는 옵션셋 테이블을 만들어 하나의 제품으로 하였습니다. <br>
          그래서 하나의 제품은 다양한 제품의 옵션을 가질 수 있고, <br>
          모델이나 카테고리에 제약받지 않는 자유도 높은 옵션을 생성할 수 있게 되었습니다. <br>

          <p>
          JPA 기술을 이용해 데이터를 매핑했고, 검색할 때는 선택한 옵션만 검색 조건에 포함시켜 동적으로 JPQL 쿼리문을 생성할 수 있게 쿼리 생성기를 만들었습니다. 
          </p>
          
          <p>프론트에는 BootStrap 템플릿을 기본으로 하고, Thymeleaf layout을 적용하였습니다. <br>
          비동기 방식으로 통신하는 경우에는 Handlerbars를 사용하면서 SSR과 CSR을 제대로 이해할 수 있었습니다. <br>

          또한, 배포를 경험해보기 위해 AWS서비스들의 개념을 공부하고 elasticbeanstalk을 이용해 배포 후<br>
